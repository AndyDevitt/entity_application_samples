...
object Approve {
  ...
  final case class ApproveCmd[F[_]](userId: UserId,
    ...
    override def action(invoice: Invoice, permissions: InvoiceUserPermissions): Either[InvoiceError, Invoice] =
      Approve.ApproveCmdProcessor().process(invoice, this, permissions)
  ...
  case class ApproveCmdProcessor[F[_]]() extends InvoiceEntityCommandProcessor[F, Invoice, InvoiceAction.Approve.type, ApproveCmd[F]] {
    ...
    override protected def action(entity: Invoice, cmd: ApproveCmd[F], permissions: InvoiceUserPermissions): Either[InvoiceError, Invoice] =
