...
sealed trait InvoiceCreateCommand[F[_], H[_]] extends EntityCreateCommand[F, H, DomainCommandInput[F], InvoiceError, InvoiceId, Invoice, InvoiceUserPermissions] {
...
sealed trait InvoiceQueryCommand[F[_], H[_], R] extends EntityQueryCommand[F, H, DomainCommandInput[F], InvoiceError, InvoiceId, Invoice, R, InvoiceRepo[F], InvoiceUserPermissions] {
  override def extractRepo(input: DomainCommandInput[F]): InvoiceRepo[F] = input.invoiceRepo
}

sealed trait InvoiceUpdateCommand[F[_], H[_], CmdType] extends EntityUpdateCommand[F, H, DomainCommandInput[F], InvoiceError, InvoiceId, Invoice, InvoiceUserPermissions] {
...
final case class InvoiceRetrieveCommand[F[_], H[_]](userId: UserId, id: InvoiceId) extends EntityRetrieveCommand[F, H, DomainCommandInput[F], InvoiceError, InvoiceId, Invoice, InvoiceUserPermissions] {
  override def extractRepo(input: DomainCommandInput[F]): EntityRepo[F, InvoiceId, Invoice, InvoiceError] = input.invoiceRepo
}
...
final case class ApproveCmd[F[_], H[_]](userId: UserId, id: InvoiceId, version: EntityVersion) extends InvoiceUpdateCommand[F, H, ApproveCmd[F, H]] {
  override def action(invoice: Invoice, permissions: InvoiceUserPermissions): Either[InvoiceError, Invoice] = InvoiceAlgebra.approve[F, H](invoice, this, permissions)

  override def permissionsRetriever: EntityPermissionsRetriever[H, InvoiceId, Invoice, InvoiceUserPermissions] = ???
}
...
final case class ApproveCmdV2[F[_], H[_]](userId: UserId, id: InvoiceId, version: EntityVersion) extends InvoiceUpdateCommand[F, H, ApproveCmdV2[F, H]] {
  override def action(invoice: Invoice, permissions: InvoiceUserPermissions): Either[InvoiceError, Invoice] = InvoiceAlgebra.approveV2[F, H](invoice, this, permissions)
...
final case class UpdateRfiCmd[F[_], H[_]](userId: UserId, id: InvoiceId, version: EntityVersion) extends InvoiceUpdateCommand[F, H, UpdateRfiCmd[F, H]] {
  override def action(invoice: Invoice, permissions: InvoiceUserPermissions): Either[InvoiceError, Invoice] = InvoiceAlgebra.updateRfi(invoice, this, permissions)
