...
object InvoiceView {
  def create(invoice: Invoice, permissions: InvoiceUserPermissions): Either[InvoiceError, InvoiceView] =
    for {
      ...
      total = total,
      actions = InvoiceAlgebra.actionStatuses(invoice, permissions)
    )
